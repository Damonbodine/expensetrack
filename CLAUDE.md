# Brownfield Agent — expensetrack

**Run ID:** n9739x6q790xre72vhyrnep8gh83sqdn
**Project ID:** jd7d2n730n1ntwcfak1dedxad983mm4a
**Repo:** /Users/damonbodine/Factory512/builds/expensetrack-20260326-185133/expensetrack
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Receipt Smart Parser — on expense form, user describes purchase in plain text, AI extracts: amount, vendor, date, category, purpose, flags if receipt needed. Auto-fills form fields. 2) Expense Report Narrative — on expense report detail, button to generate narrative summary of all expenses in the report for finance committee review. 3) Policy Compliance Checker — when expense submitted, AI checks against org policies (per-diem limits, pre-approval requirements, documentation). Returns pass/warning/fail with policy references. 4) Budget Variance Explainer — on budget page, when actuals diverge from budget, AI generates explanation of why with specific line-item drivers. Each feature needs: Convex action, React component, integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/expensetrack-20260326-185133/expensetrack. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n9739x6q790xre72vhyrnep8gh83sqdn"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n9739x6q790xre72vhyrnep8gh83sqdn"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n9739x6q790xre72vhyrnep8gh83sqdn","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n9739x6q790xre72vhyrnep8gh83sqdn","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n9739x6q790xre72vhyrnep8gh83sqdn","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/expensetrack-20260326-185133/expensetrack`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Receipt Smart Parser — on expense form, user describes purchase in plain text, AI extracts: amount, vendor, date, category, purpose, flags if receipt needed. Auto-fills form fields. 2) Expense Report Narrative — on expense report detail, button to generate narrative summary of all expenses in the report for finance committee review. 3) Policy Compliance Checker — when expense submitted, AI checks against org policies (per-diem limits, pre-approval requirements, documentation). Returns pass/warning/fail with policy references. 4) Budget Variance Explainer — on budget page, when actuals diverge from budget, AI generates explanation of why with specific line-item drivers. Each feature needs: Convex action, React component, integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n9739x6q790xre72vhyrnep8gh83sqdn",
  "projectName": "expensetrack",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Receipt Smart Parser — on expense form, user describes purchase in plain text, AI extracts: amount, vendor, date, category, purpose, flags if receipt needed. Auto-fills form fields. 2) Expense Report Narrative — on expense report detail, button to generate narrative summary of all expenses in the report for finance committee review. 3) Policy Compliance Checker — when expense submitted, AI checks against org policies (per-diem limits, pre-approval requirements, documentation). Returns pass/warning/fail with policy references. 4) Budget Variance Explainer — on budget page, when actuals diverge from budget, AI generates explanation of why with specific line-item drivers. Each feature needs: Convex action, React component, integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/expensetrack-20260326-185133/expensetrack`
